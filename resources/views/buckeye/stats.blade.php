    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-white">Your BuckEYE Stats</h1>
                    <p class="text-steel-400 text-sm">Track your progress and achievements</p>
                </div>
            </div>
            <x-nav-link href="{{ route('buckeye.index') }}">
                ← Back to Game
            </x-nav-link>
        </div>
    </x-slot>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <article class="bg-gradient-to-br from-steel-800 to-steel-850 rounded-xl border border-steel-700/50 overflow-hidden">
                    ...
                    <div class="p-6">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            ...
                            <div class="text-center p-4 bg-gradient-to-br from-amber-500/20 to-amber-600/10 rounded-xl border border-amber-500/30">
                                <div class="text-3xl font-bold text-amber-400">{{ $userStats->max_streak }}</div>
                                <div class="text-sm text-amber-300/70">Best Streak</div>
                            </div>
                        </div>
                        ...
                        <div class="mt-6">
                            ...
                            <div class="bg-steel-700/50 rounded-full h-4 overflow-hidden">
                                <div
                                    class="bg-gradient-to-r from-amber-500 to-amber-400 h-4 rounded-full transition-all duration-500"
                            ...
                            <div class="space-y-3">
                                @foreach(range(1, 5) as $guessNumber)
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        <div class="flex-1">
                                            <div
                                                class="bg-gradient-to-r from-amber-500 to-amber-400 text-right px-2 py-1 text-sm font-bold rounded text-steel-900"
                ...
                <article class="bg-gradient-to-br from-steel-800 to-steel-850 rounded-xl border border-steel-700/50 overflow-hidden">
                    ...
                    <div class="p-6">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            @php $eagleEyeEarned = isset($userStats->guess_distribution[1]) && $userStats->guess_distribution[1] > 0; @endphp
                            <div class="{{ $eagleEyeEarned ? 'bg-gradient-to-br from-amber-500/10 to-amber-600/5 border-amber-500/30 ring-2 ring-amber-400/20' : 'bg-steel-800/50 border-steel-700/30 opacity-50' }} rounded-xl p-4 text-center border">
                                <div class="w-12 h-12 {{ $eagleEyeEarned ? 'bg-amber-500 shadow-lg shadow-amber-500/30' : 'bg-steel-700' }} rounded-full mx-auto mb-3 flex items-center justify-center">
                                    <svg class="w-6 h-6 {{ $eagleEyeEarned ? 'text-white' : 'text-steel-500' }}" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                                    </svg>
                                </div>
                                <h3 class="font-semibold {{ $eagleEyeEarned ? 'text-white' : 'text-steel-400' }}">Eagle Eye</h3>
                                <p class="text-xs {{ $eagleEyeEarned ? 'text-amber-300/70' : 'text-steel-500' }} mt-1">Win on first guess</p>
                            </div>

                            @php $onFireEarned = $userStats->current_streak >= 3; @endphp
                            <div class="{{ $onFireEarned ? 'bg-gradient-to-br from-amber-500/10 to-amber-600/5 border-amber-500/30 ring-2 ring-amber-400/20' : 'bg-steel-800/50 border-steel-700/30 opacity-50' }} rounded-xl p-4 text-center border">
                                <div class="w-12 h-12 {{ $onFireEarned ? 'bg-amber-500 shadow-lg shadow-amber-500/30' : 'bg-steel-700' }} rounded-full mx-auto mb-3 flex items-center justify-center">
                                ...
                                <p class="text-xs {{ $onFireEarned ? 'text-amber-300/70' : 'text-steel-500' }} mt-1">Get a 3-day streak</p>
                            </div>

                            @php $faithfulEarned = $userStats->max_streak >= 10; @endphp
                            <div class="{{ $faithfulEarned ? 'bg-gradient-to-br from-amber-500/10 to-amber-600/5 border-amber-500/30 ring-2 ring-amber-400/20' : 'bg-steel-800/50 border-steel-700/30 opacity-50' }} rounded-xl p-4 text-center border">
                                <div class="w-12 h-12 {{ $faithfulEarned ? 'bg-amber-500 shadow-lg shadow-amber-500/30' : 'bg-steel-700' }} rounded-full mx-auto mb-3 flex items-center justify-center">
                                ...
                                <p class="text-xs {{ $faithfulEarned ? 'text-amber-300/70' : 'text-steel-500' }} mt-1">10-day streak</p>
                            </div>

                            @php $expertEarned = $userStats->games_won >= 100; @endphp
                            <div class="{{ $expertEarned ? 'bg-gradient-to-br from-amber-500/10 to-amber-600/5 border-amber-500/30 ring-2 ring-amber-400/20' : 'bg-steel-800/50 border-steel-700/30 opacity-50' }} rounded-xl p-4 text-center border">
                                <div class="w-12 h-12 {{ $expertEarned ? 'bg-amber-500 shadow-lg shadow-amber-500/30' : 'bg-steel-700' }} rounded-full mx-auto mb-3 flex items-center justify-center">
                                    <svg class="w-6 h-6 {{ $expertEarned ? 'text-white' : 'text-steel-500' }}" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"/>
                                    </svg>
                                </div>
                                <h3 class="font-semibold {{ $expertEarned ? 'text-white' : 'text-steel-400' }}">Ohio Expert</h3>
                                <p class="text-xs {{ $expertEarned ? 'text-amber-300/70' : 'text-steel-500' }} mt-1">Win 100 games</p>
                            </div>
                        </div>
                    </div>
                </article>
                ...
                <div class="p-6">
                                        ...
                                        <div class="flex flex-col md:flex-row gap-4">
                                            ...
                                            <div class="w-full md:w-32 shrink-0">
                                                @if($puzzle->image_path)
                                                    ...
                                                    >
                                                @else
                                                    <div class="w-full h-24 md:h-32 bg-steel-600/50 rounded-lg flex items-center justify-center">
                                                        <span class="text-steel-400 text-sm">No image</span>
                                                    </div>
                                                @endif
                                            </div>
                                            ...
                                            <div class="flex-1">
                                                <div class="flex justify-between items-start mb-2">
                                                    <div>
                                                        <h3 class="text-xl font-bold text-amber-400">{{ $puzzle->answer }}</h3>
                                                        <p class="text-steel-400 text-sm">{{ $puzzle->publish_date->format('F j, Y') }}</p>
                                                    </div>
                                                    @if($progress->solved)
                                                    ...
                                                    <div class="mt-3">
                                                        ...
                                                        <div class="flex flex-wrap gap-2">
                                                            @foreach($progress->previous_guesses as $index => $guess)
                                                                <span class="px-3 py-1 text-sm rounded-lg {{ $index == array_key_last($progress->previous_guesses) && $progress->solved ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'bg-steel-600/50 text-steel-300 border border-steel-500/30' }}">
                                                                    {{ $guess }}
                                                                </span>
                                                            @endforeach
                                                        </div>
                                                ...
                                                @if($puzzle->hint)
                                                    <div class="mt-3 text-sm text-steel-400">
                                                        <span class="text-steel-500">Hint:</span> {{ $puzzle->hint }}
                                                    </div>
                                                @endif

                                                @if($puzzle->link)
                                                    <div class="mt-3">
                                                        <a href="{{ $puzzle->link }}" target="_blank" class="text-accent-400 hover:text-accent-300 text-sm transition-colors">
                    ...
                    @else
                        <div class="py-12 text-center text-steel-400">
                            ...
                            <a href="{{ route('buckeye.index') }}" class="inline-block mt-4">
