                ...
                <span>
                    {{ \Carbon\Carbon::parse($thread->created_at)->setTimezone((auth()->check() ? auth()->user()->timezone : null))->diffForHumans() }}
                     <a href="/profiles/{{ $thread->owner->username }}"
                        class="text-blue-500 hover:underline">{{ $thread->owner->username }}</a>
                </span>
            @endif
        </div>

        <div
            class="flex items-center justify-end space-x-2 bg-main-color posted-by-when rounded shadow md:shadow-none py-1.5 md:p-2 md:p-0 md:m-0">
            @if ($thread->lastReply)
                <div class="text-right">
                    ...
                    <a href="/profiles/{{ $thread->lastReply->owner->username }}"
                       class="text-blue-500 hover:underline">{{ $thread->lastReply->owner->username }}</a>
                </div>
