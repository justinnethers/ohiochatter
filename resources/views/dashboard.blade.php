    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('thread.index') }}"
               class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-accent-500 to-accent-600 rounded-lg text-white font-semibold text-sm shadow-lg shadow-accent-500/25 hover:shadow-accent-500/40 hover:from-accent-600 hover:to-accent-700 transition-all duration-200 whitespace-nowrap shrink-0">
                ...
                <div class="space-y-6">
                    ...
                    <div
                        ...
                        <div class="mt-4 pt-4 border-t border-steel-700/50 grid grid-cols-2 gap-4 text-center">
                            <div>
                                <div
                                    class="text-lg font-bold text-white">{{ number_format($user->profile_views) }}</div>
                                <div class="text-steel-400 text-xs">Profile Views</div>
                            </div>
                            <div>
                                <div
                                    class="text-lg font-bold text-white">{{ number_format($user->dashboard_visits) }}</div>
                    ...
                    <div
                        ...
                        @if($messageThreads->count() > 0)
                                ...
                                @foreach($messageThreads->take(3) as $thread)
                                    <a href="{{ route('messages.show', $thread) }}"
                                       class="block p-3 rounded-lg bg-steel-900/50 border border-steel-700/30 hover:border-steel-600 hover:bg-steel-900 transition-all duration-200 group">
                                        <div
                                            class="font-medium text-white group-hover:text-accent-400 transition-colors text-sm truncate">
                                            {{ $thread->subject }}
                                        </div>
                            ...
                            <a href="{{ route('messages.index') }}"
                               class="block mt-4 text-center text-accent-400 hover:text-accent-300 text-sm transition-colors">
                                View all messages
                            </a>
                        @else
                    ...
                    @if($wordleStats && $wordleStats->games_played > 0)
                        <div
                            class="bg-gradient-to-br from-steel-800 to-steel-850 p-6 rounded-xl shadow-lg shadow-black/20 border border-steel-700/50">
                            <div class="flex items-center justify-between mb-4">
                                <h3 class="text-lg font-semibold text-white">Wordio Stats</h3>
                                <a href="{{ route('ohiowordle.index') }}"
                                   class="text-accent-400 hover:text-accent-300 text-sm transition-colors">
                                    Play
                                </a>
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div class="text-center p-3 rounded-lg bg-steel-900/50">
                                    <div class="text-2xl font-bold text-white">{{ $wordleStats->games_won }}</div>
                                    <div class="text-steel-400 text-xs">Wins</div>
                                </div>
                                <div class="text-center p-3 rounded-lg bg-steel-900/50">
                                    <div
                                        class="text-2xl font-bold text-emerald-400 flex items-center justify-center gap-1">
                            ...
                            <div class="mt-4 pt-4 border-t border-steel-700/50 flex justify-between text-sm">
                                <span class="text-steel-400">Win Rate</span>
                                <span class="text-emerald-400 font-medium">
                                    {{ number_format(($wordleStats->games_won / $wordleStats->games_played) * 100, 1) }}%
                                </span>
                            </div>
                        </div>
                    @else
                        <div
                            class="bg-gradient-to-br from-steel-800 to-steel-850 p-6 rounded-xl shadow-lg shadow-black/20 border border-steel-700/50">
                            <h3 class="text-lg font-semibold text-white mb-4">Wordio</h3>
                            <p class="text-steel-400 text-sm mb-4">Ohio's daily word puzzle!</p>
                            <a href="{{ route('ohiowordle.index') }}"
                               class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-emerald-500 to-emerald-600 rounded-lg text-white font-semibold text-sm shadow-lg hover:from-emerald-600 hover:to-emerald-700 transition-all duration-200">
                                Play Now
                            </a>
                        </div>
                    @endif
                    ...
                    <div
                        ...
                        @else
                            <div class="text-center py-8 text-steel-400">
                                <svg class="w-12 h-12 mx-auto mb-3 text-steel-600" fill="none" stroke="currentColor"
                                     viewBox="0 0 24 24">
                        ...
                        <div
                            ...
                            <div class="space-y-3">
                                @foreach($recentRepActivity as $activity)
                                    <div
                                        class="flex items-center gap-4 p-3 rounded-lg bg-steel-900/50 border border-steel-700/30">
                                        <div class="flex-shrink-0">
                                            @if($activity['type'] === 'rep')
                                                <div
                                                    class="w-10 h-10 rounded-full bg-emerald-500/20 flex items-center justify-center">
                                                    <svg class="w-5 h-5 text-emerald-400" fill="currentColor"
                                                         viewBox="0 0 20 20">
                                                        <path fill-rule="evenodd"
                                                              d="M3.293 9.707a1 1 0 010-1.414l6-6a1 1 0 011.414 0l6 6a1 1 0 01-1.414 1.414L11 5.414V17a1 1 0 11-2 0V5.414L4.707 9.707a1 1 0 01-1.414 0z"
                                                              clip-rule="evenodd"></path>
                                            ...
                                            @else
                                                <div
                                                    ...
                                                    <svg class="w-5 h-5 text-red-400" fill="currentColor"
                                                         viewBox="0 0 20 20">
                                                        <path fill-rule="evenodd"
                                                              d="M16.707 10.293a1 1 0 010 1.414l-6 6a1 1 0 01-1.414 0l-6-6a1 1 0 111.414-1.414L9 14.586V3a1 1 0 012 0v11.586l4.293-4.293a1 1 0 011.414 0z"
                                                              clip-rule="evenodd"></path>
                                        ...
                                        <div class="flex-1 min-w-0">
                                            <p class="text-white text-sm">
                                                <a href="{{ route('profile.show', ['user' => $activity['username']]) }}"
                                                   class="font-medium hover:text-accent-400 transition-colors">
                                                    {{ $activity['username'] }}
                                                </a>
                                            ...
                                            <a href="{{ $replyUrl }}"
                                               class="text-steel-400 text-xs hover:text-accent-400 transition-colors truncate block">
