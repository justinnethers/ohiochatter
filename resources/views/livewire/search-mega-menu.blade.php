        ...
        >
            @if($this->hasResults())
                <div class="max-h-[70vh] overflow-y-auto">
                    <div
                        class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-0 divide-y md:divide-y-0 md:divide-x divide-steel-700/50">
                                        ...
                                        @foreach($results['threads'] as $thread)
                                            <a href="{{ $thread->path() }}"
                                               class="block p-2 rounded-lg hover:bg-steel-700/50 transition-colors group">
                                                <div
                                                    class="text-sm text-white group-hover:text-accent-300 font-medium line-clamp-1">{{ $thread->title }}</div>
                                                <div
                                                    class="text-xs text-steel-500">{{ $thread->forum->name ?? 'Forum' }}</div>
                                            </a>
                                        @endforeach
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div class="space-y-1">
                                        @foreach($results['posts'] as $post)
                                            <a href="{{ $post->thread?->path() }}#post-{{ $post->id }}"
                                               class="block p-2 rounded-lg hover:bg-steel-700/50 transition-colors group">
                                                <div
                                                    class="text-sm text-steel-200 group-hover:text-white line-clamp-1">{{ Str::limit(strip_tags($post->body), 60) }}</div>
                                                <div class="text-xs text-steel-500">
                                                    in
                                                    {{ $post->thread?->title ?? 'Thread' }} &bull; {{ $post->owner?->username ?? 'Unknown' }}
                                                </div>
                                            </a>
                                        @endforeach
                                    </div>
                                        ...
                                        @foreach($results['users'] as $user)
                                            <a href="{{ route('profile.show', $user) }}"
                                               class="flex items-center gap-2 p-2 rounded-lg hover:bg-steel-700/50 transition-colors group">
                                                ...
                                                <span
                                                    class="text-sm text-white group-hover:text-accent-300 font-medium">{{ $user->username }}</span>
                                            </a>
                                        @endforeach
                                    </div>
                                </div>
                                        ...
                                        @foreach($results['archive'] as $item)
                                            @if($item['type'] === 'thread')
                                                <a href="{{ route('archive.thread', $item['item']) }}"
                                                   class="block p-2 rounded-lg hover:bg-steel-700/50 transition-colors group">
                                                    <div
                                                        class="text-sm text-white group-hover:text-accent-300 font-medium line-clamp-1">{{ $item['item']->title }}</div>
                                                    <div
                                                        class="text-xs text-steel-500">{{ $item['item']->forum?->title ?? 'Archive' }}</div>
                                                </a>
                                            @else
                                                <a href="{{ route('archive.thread', $item['item']->thread) }}"
                                                   class="block p-2 rounded-lg hover:bg-steel-700/50 transition-colors group">
                                                    <div
                                                        class="text-sm text-steel-200 group-hover:text-white line-clamp-1">{{ Str::limit(strip_tags($item['item']->pagetext), 60) }}</div>
                                                    <div class="text-xs text-steel-500">
                                                        in {{ $item['item']->thread?->title ?? 'Thread' }}</div>
                                                </a>
                                            @endif
                                        @endforeach
                                    </div>
                                    ...
                                    <div class="space-y-1">
                                        @foreach($results['guides'] as $guide)
                                            <a href="{{ route('guide.show', $guide) }}"
                                               class="block p-2 rounded-lg hover:bg-steel-700/50 transition-colors group">
                                                <div
                                                    class="text-sm text-white group-hover:text-accent-300 font-medium line-clamp-1">{{ $guide->title }}</div>
                                                @if($guide->location_name)
                                                    <div
                                                        class="text-xs text-steel-500">{{ $guide->location_name }}</div>
                                                @endif
                                            </a>
                                        @endforeach
                                    </div>
                                    ...
                                    <div class="space-y-1">
                                        @foreach($results['locations'] as $location)
                                            @if($location['type'] === 'region')
                                                <a href="{{ route('region.show', $location['item']) }}"
                                                   class="flex items-center gap-2 p-2 rounded-lg hover:bg-steel-700/50 transition-colors group">
                                                    <span
                                                        class="text-xs px-1.5 py-0.5 bg-emerald-500/20 text-emerald-400 rounded font-medium">Region</span>
                                                    <span
                                                        class="text-sm text-white group-hover:text-accent-300">{{ $location['item']->name }}</span>
                                                </a>
                                            @else
                                                <a href="{{ route('city.show', ['region' => $location['item']->county->region, 'county' => $location['item']->county, 'city' => $location['item']]) }}"
                                                   class="flex items-center gap-2 p-2 rounded-lg hover:bg-steel-700/50 transition-colors group">
                                                    <span
                                                        class="text-xs px-1.5 py-0.5 bg-blue-500/20 text-blue-400 rounded font-medium">City</span>
                                                    <span
                                                        class="text-sm text-white group-hover:text-accent-300">{{ $location['item']->name }}</span>
                                                </a>
                                            @endif
