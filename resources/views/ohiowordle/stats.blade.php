            ...
            <div class="max-w-6xl mx-auto space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <x-well class="bg-gray-800 p-6 rounded-md">
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            ...
                            <div class="text-center p-3 bg-red-600 text-white rounded-lg">
                                        ...
                                        @foreach(range(1, 6) as $guessNumber)
                                            ...
                                            <div class="flex items-center">
                                                ...
                                                <div class="flex-1 ml-2">
                                                    <div
                                                        class="bg-red-600 text-white text-right px-2 py-1 text-sm font-bold rounded-sm"
                    ...
                    <article>
                        ...
                        <div class="p-2 bg-gray-700 rounded-b-md">
                            <div class="bg-gray-800 p-2 rounded-md">
                                <div class="grid grid-cols-2 gap-2">
                                    <div class="bg-gray-700 rounded-lg p-2 text-center {{ isset($userStats->guess_distribution[1]) && $userStats->guess_distribution[1] > 0 ? 'opacity-100' : 'opacity-40' }}">
                                        ...
                                        <h3 class="font-medium">First Try</h3>
                                        <p class="text-xs text-gray-400 mt-1">Win on first guess</p>
                                    </div>

                                    <div class="bg-gray-700 rounded-lg p-2 text-center {{ $userStats->current_streak >= 3 ? 'opacity-100' : 'opacity-40' }}">
                                        <div class="w-12 h-12 bg-gray-600 rounded-full mx-auto mb-3 flex items-center justify-center">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-gray-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    ...
                                    <div class="bg-gray-700 rounded-lg p-2 text-center {{ $userStats->max_streak >= 10 ? 'opacity-100' : 'opacity-40' }}">
                                        <div class="w-12 h-12 bg-gray-600 rounded-full mx-auto mb-3 flex items-center justify-center">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-gray-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    ...
                                    <div class="bg-gray-700 rounded-lg p-2 text-center {{ $userStats->games_won >= 100 ? 'opacity-100' : 'opacity-40' }}">
                                        <div class="w-12 h-12 bg-gray-600 rounded-full mx-auto mb-3 flex items-center justify-center">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-gray-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"/>
                                            </svg>
                                        </div>
                                        <h3 class="font-medium">Word Master</h3>
                ...
                <article>
                    <div class="bg-gray-600 text-white p-2 px-4 rounded-t-md">
                        <h2 class="text-lg font-semibold">Past Words</h2>
                    </div>
                    <div class="p-2 bg-gray-700 rounded-b-md">
                        <div class="bg-gray-800 p-2 rounded-md">
                            @php
                                            ...
                                            <div class="bg-gray-700 rounded-md p-4">
                                                <div class="flex justify-between items-start mb-3">
                                                    <div>
                                                        ...
                                                        <p class="text-gray-400 text-sm">{{ $word->publish_date->format('F j, Y') }}</p>
                                                    </div>
                                                    <div>
                                                        @if($progress->solved)
                                                ...
                                                @if($progress->guesses && count($progress->guesses) > 0)
                                                    <div>
                                                        <h4 class="text-sm font-semibold text-gray-300 mb-2">Your Guesses:</h4>
                                                            ...
                                                            @foreach($progress->guesses as $index => $guess)
                                                                <div class="flex gap-px">
                                                                    @foreach(str_split($guess) as $letterIndex => $letter)
                                                                        ...
                                                                        @endphp
                                                                        <span class="w-6 h-6 text-xs font-bold flex items-center justify-center rounded {{ $bgColor }} text-white">
                                                                            {{ strtoupper($letter) }}
                                                                        </span>
                                                                    @endforeach
                                                                </div>
                                                ...
                                                @if($word->hint)
                                                    <div class="mt-3 text-sm">
                                                        <span class="text-gray-400">Hint: </span>{{ $word->hint }}
                                                    </div>
                                                @endif
                                            </div>
                                        @endif
